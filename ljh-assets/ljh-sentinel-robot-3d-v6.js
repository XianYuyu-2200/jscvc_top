...
const rollerGeo = new THREE.CylinderGeometry(0.054, 0.054, 0.52, 14);
...
function createOmniWheel(position, handedness) {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    roller.position.copy(radial.multiplyScalar(0.46));
...
const projectileGeo = new THREE.SphereGeometry(0.07, 18, 18);
const projectiles = [];
const fireIntervalMs = 170;
let fireTimer = 0;
let flashPower = 0;
let recoil = 0;
...
function fireShot() {
  ...
  projectiles.push({
    mesh: shotMesh,
    trail,
    velocity: shotDirection.clone().multiplyScalar(10.6),
    age: 0,
    maxAge: 1.15
  });
