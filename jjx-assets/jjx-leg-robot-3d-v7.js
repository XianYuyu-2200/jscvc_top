...
function animate() {
  ...
  const jumpTuck = THREE.MathUtils.clamp(jumpOffset * 2.2 + Math.max(0, jumpVelocity) * 0.12, 0, 0.72);
  ...
  legs.forEach((leg) => {
    ...
    const balance = -bodyLift + pointerY * 0.035 + jumpSquash * 0.12 + jumpTuck;
    const stanceShift = pointerX * 0.035;
    solveSerialLeg(leg, balance, stanceShift);
    leg.wheel.rotation.z = 0;
    leg.hub.rotation.z = 0;
    leg.footLight.intensity = 1.45 + crouch * 1.8 + jumpSquash * 3.4 + jumpTuck * 3.2 + Math.max(0, -balance) * 7.2;
