...
function createArmorPlate(parent, size, position, rotation, accentMaterial, lightPosition) {
...
box(body, [3.25, 0.62, 1.36], materials.armor, [0, 0.2, 0]);
box(body, [2.66, 0.18, 1.58], materials.dark, [0, -0.17, 0]);
box(body, [1.42, 0.58, 0.92], materials.glass, [0.08, 0.74, 0]);
box(body, [0.74, 0.32, 1.72], materials.steel, [-1.28, 0.18, 0]);
box(body, [0.92, 0.22, 1.56], materials.dark, [1.22, 0.5, 0]);
glowBox(body, [1.18, 0.045, 0.055], materials.cyanLine, [-0.72, 0.55, 0.72]);
glowBox(body, [1.18, 0.045, 0.055], materials.amberLine, [0.72, 0.55, 0.72]);
glowBox(body, [1.26, 0.045, 0.055], materials.redLine, [0, 0.04, -0.72]);

createArmorPlate(body, [0.08, 0.42, 0.82], [1.7, 0.22, 0], [0, 0, 0], materials.cyanLine, [0.046, 0, 0]);
createArmorPlate(body, [0.08, 0.42, 0.82], [-1.7, 0.22, 0], [0, 0, 0], materials.amberLine, [-0.046, 0, 0]);
createArmorPlate(body, [0.86, 0.42, 0.08], [0.18, 0.22, 0.82], [0, 0, 0], materials.cyanLine, [0, 0, 0.046]);
createArmorPlate(body, [0.86, 0.42, 0.08], [0.18, 0.22, -0.82], [0, 0, 0], materials.amberLine, [0, 0, -0.046]);

const turret = new THREE.Group();
turret.position.set(0.22, 2.22, 0);
robot.add(turret);

const turretBase = cylinder(turret, 0.72, 0.82, 0.42, materials.armor, [0, 0, 0], [0, Math.PI / 10, 0], 48);
const turretHead = box(turret, [1.32, 0.68, 0.94], materials.dark, [0, 0.42, 0]);
const barrel = cylinder(turret, 0.12, 0.16, 2.18, materials.dark, [1.48, 0.42, 0], [0, 0, Math.PI / 2], 28);
const muzzle = cylinder(turret, 0.2, 0.2, 0.14, materials.cyanLine, [2.6, 0.42, 0], [0, 0, Math.PI / 2], 28);

const muzzleFlash = new THREE.Mesh(new THREE.SphereGeometry(0.22, 18, 18), materials.flash);
muzzleFlash.position.set(2.72, 0.42, 0);
...
function createLeg(side) {
  ...
  link(hip, upperLen, materials.steel, -0.06, -0.08);
  glowBox(hip, [upperLen * 0.82, 0.035, 0.04], materials.amberLine, [upperLen * 0.45, 0.16, 0]);
  ...
  link(knee, lowerLen, materials.steel, -0.06, -0.08);
  glowBox(knee, [lowerLen * 0.78, 0.035, 0.04], materials.cyanLine, [lowerLen * 0.42, -0.16, 0]);
  ...
  const innerRim = new THREE.Mesh(rimGeo, materials.amberLine);
  ...
  const footLight = new THREE.PointLight(side > 0 ? 0x45d8ff : 0xffd166, 1.8, 2.8);
...
const balanceRing = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ color: 0xffd166, transparent: true, opacity: 0.56 })
);
...
for (let i = 0; i < 86; i += 1) {
  const dot = new THREE.Mesh(dotGeo, i % 4 === 0 ? materials.amberLine : materials.cyanLine);
...
const key = new THREE.PointLight(0x45d8ff, 7.2, 18);
key.position.set(-4.8, 4.4, 5.2);
scene.add(key);

const rim = new THREE.PointLight(0xffd166, 5.4, 14);
...
function animate() {
  ...
  barrel.position.x = 1.48 - 0.18 * recoil;
  muzzle.position.x = 2.6 - 0.22 * recoil;
  muzzleFlash.position.x = 2.72 - 0.22 * recoil;
  ...
  turret.position.y = 2.22 + bodyLift;
