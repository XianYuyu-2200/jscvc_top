...
function createLeg(side) {
  ...
  const shoulderSleeve = box(sideGroup, [0.16, 0.76, 0.14], materials.steel, [-0.58, 0.66, 0]);
  glowBox(sideGroup, [0.045, 0.58, 0.04], materials.redLine, [-0.48, 0.64, 0.09]);
  ...
  cylinder(wheelGroup, 0.32, 0.32, 0.045, materials.redLine, [0, 0.22, 0], [Math.PI / 2, 0, 0], 36);
  cylinder(wheelGroup, 0.32, 0.32, 0.045, materials.redLine, [0, -0.22, 0], [Math.PI / 2, 0, 0], 36);
  ...
  legs.push({
    ...
    baseHip,
    baseKnee,
    shoulderBasePosition: shoulder.position.clone(),
    shoulderSleeveBasePosition: shoulderSleeve.position.clone(),
    hipBasePosition: hip.position.clone()
  });
}
...
function onKeyDown(event) {
  ...
  if (key === 'x' && jumpOffset <= 0.001 && jumpVelocity === 0) {
    jumpVelocity = 2.75;
    jumpSquash = 1;
  }
}
...
function animate() {
  ...
  const moveX = (movementKeys.d ? 1 : 0) - (movementKeys.a ? 1 : 0);
  const moveZ = (movementKeys.s ? 1 : 0) - (movementKeys.w ? 1 : 0);
  const moveLength = Math.hypot(moveX, moveZ) || 1;
  const moveSpeed = 1.55 * (crouch > 0.45 ? 0.48 : 1);
  robotGroundX = THREE.MathUtils.clamp(robotGroundX + (moveX / moveLength) * moveSpeed * delta, -2.45, 2.45);
  robotGroundZ = THREE.MathUtils.clamp(robotGroundZ + (moveZ / moveLength) * moveSpeed * delta, -1.75, 1.75);
  jumpSquash = Math.max(0, jumpSquash - delta * 8.5);
  if (jumpVelocity !== 0 || jumpOffset > 0) {
    jumpVelocity -= 6.6 * delta;
  ...
  robot.position.set(robotGroundX, jumpOffset, robotGroundZ);
  const jointTuck = THREE.MathUtils.clamp(jumpOffset * 1.65 + Math.max(0, jumpVelocity) * 0.08, 0, 0.58);
  ...
  bodyLift = idleLift - crouch * 0.42 - jumpSquash * 0.18;
  body.position.y = 1.18 + bodyLift;
  turret.position.y = 2.04 + bodyLift;

  legs.forEach((leg) => {
    leg.sideGroup.position.y = bodyLift;
    const balance = -bodyLift + pointerY * 0.035 + jumpSquash * 0.12;
    const stanceShift = pointerX * 0.035;
    const shoulderCompress = THREE.MathUtils.clamp(jointTuck + jumpSquash * 0.22 + crouch * 0.16, 0, 0.7);
    leg.shoulder.position.x = leg.shoulderBasePosition.x + shoulderCompress * 0.11;
    leg.shoulder.position.y = leg.shoulderBasePosition.y + shoulderCompress * 0.2;
    leg.shoulder.scale.set(
      1 - shoulderCompress * 0.16,
      1 - shoulderCompress * 0.26,
      1 - shoulderCompress * 0.16
    );
    leg.shoulderSleeve.position.y = leg.shoulderSleeveBasePosition.y + shoulderCompress * 0.16;
    leg.shoulderSleeve.scale.y = Math.max(0.54, 1 - shoulderCompress * 0.48);
    leg.hip.position.x = leg.hipBasePosition.x + shoulderCompress * 0.11;
    leg.hip.position.y = leg.hipBasePosition.y + shoulderCompress * 0.2;
    solveSerialLeg(leg, balance, stanceShift);
    leg.hip.rotation.z -= jointTuck * 0.22;
    leg.knee.rotation.z += jointTuck * 0.95;
    leg.wheelGroup.rotation.z = -(leg.hip.rotation.z + leg.knee.rotation.z);
    leg.wheel.rotation.z = 0;
    leg.hub.rotation.z = 0;
    leg.footLight.intensity = 1.45 + crouch * 1.8 + jumpSquash * 3.4 + jointTuck * 3.2 + Math.max(0, -balance) * 7.2;
